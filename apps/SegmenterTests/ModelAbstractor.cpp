...
void ModelAbstractor::run(std::string _rgbd_filename,
                          ...
                          int _startIdx, int _endIdx, bool _live)
{
  ...
  while(do_it) {
    ...
    if (key == 65478 || key == 1114054)  { // F9
      printf("[ModelAbstractor] Process single image.\n");
      process();
      pclA::ConvertPCLCloud2Image(pcl_cloud, kImage);
      cv::imshow("Debug image", kImage);
      dbgWin.SetImage(kImage);
    ...
    if (key == 65479 || key == 1114055)  { // F10
      printf("[ModelAbstractor] Process images countiniously.\n");
      single_image = false;
      processed = true;
    }

    if((char) key == 'h') {
      printf("[ModelAbstractor] Print help:\n");
      printf("  Debug window:\n");
    ...
    if((char) key == 'p') {
      if(processed) {
        printf("[ModelAbstractor] Print patch info:\n");
    ...
    if((char) key == 's') {
      if(processed) {
        printf("[ModelAbstractor] Save model to file.\n");
        resultSaver->SaveNextView(surfaces);
      }
    }
    
    if((char) key == 'q') {
      printf("[ModelAbstractor] Quit.\n");
      do_it = false;
    }
    ...
    if((char) key == '6') {
      dbgWin.Clear();
      printf("[ModelAbstractor] Show contour of models.\n");
      pclA::RGBValue col[surfaces.size()];
      std::vector<cv::Vec4f> col_points;
      ...
      for(unsigned i=0; i<surfaces.size(); i++) {
        if(surfaces[i]->type == -1)
          ...
          col[i].float_value = pclA::GetRandomColor();
        for(unsigned j=0; j<surfaces[i]->contour.size(); j++) {
          cv::Vec4f pt;
          pt[0] = pcl_cloud->points[surfaces[i]->contour[j]].x;
          pt[1] = pcl_cloud->points[surfaces[i]->contour[j]].y;
          pt[2] = pcl_cloud->points[surfaces[i]->contour[j]].z;
          pt[3] = col[i].float_value;
          col_points.push_back(pt);
        }
      }
